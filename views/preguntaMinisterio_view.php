...
$idIndicadorME = $_GET['idIndicadorME'];


require_once dirname(dirname(__FILE__)).'/controller/preguntaMinisterioController.php';
...
$tablaPreguntaMinisterio ='';
$tablaRespuestaMinisterio ='';
foreach ($datospregunta as $key => $value) {
    $checked="";
    if($key == 0)
        $tablaPreguntaMinisterio .="<tr>
            <input type='hidden' name='idIndicadorME'  id='idIndicadorME' value='".$datospregunta[$key]['idIndicadorME']."'>
                <td>".utf8_encode($datospregunta[$key]['descripcionIndicadorME'])."</td>
                </td>
            </tr>";
    if ($datospregunta[$key]['idRespuestaIndicador'] > 0)
        $checked="checked";
        $tablaRespuestaMinisterio .="
